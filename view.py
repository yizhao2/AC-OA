...
def add_actions(name, url, view):
    try:
        ...
        if not db_tools.exec_sql(sql, values=(url), is_query=True):
            ...
            if hasattr(view, 'put'): db_tools.exec_sql(sql, values=(1, name, name, url, 'PUT', 1, 0, ''), is_query=False)
...
def register_views(api, views):
    ...
    for name, view in views_class_dict.items():
        ...
        # url1 = '/api/' + name
        # logging.warning("web register " + url1 + " interface.")
        logging.warning("web register " + url + " interface.")
